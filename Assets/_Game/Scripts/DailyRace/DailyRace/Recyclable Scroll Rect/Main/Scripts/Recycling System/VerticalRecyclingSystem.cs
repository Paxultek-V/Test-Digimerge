...
namespace PolyAndCode.UI
{
    ...
    public class VerticalRecyclingSystem : RecyclingSystem
    {
        ...
        //Trackers
        public int currentItemCount; //item count corresponding to the datasource.
        public int topMostCellIndex, bottomMostCellIndex; //Topmost and bottommost cell in the heirarchy
        ...
        private Vector2 RecycleTopToBottom()
        {
            ...
            if (IsGrid)
            {
                Content.sizeDelta += Vector2.up * (additionalRows * _cellHeight);
            ...
            _cellPool.ForEach((RectTransform cell) => cell.anchoredPosition += Vector2.up * (n * _cellPool[topMostCellIndex].sizeDelta.y));
            Content.anchoredPosition -= Vector2.up * (n * _cellPool[topMostCellIndex].sizeDelta.y);
            _recycling = false;
        ...
        public void CenterToCellInstant(int cellIndex)
        {
            int totalCells = DataSource.GetItemCount();
            int totalDatEnteries = totalCells - 1;
            int lowestIndex = cellIndex - 5;
            int highestIndex = cellIndex + 5;

            currentItemCount = highestIndex;

            Debug.Log("Leaderboard Log // Total Data Enteries is " + lowestIndex);
            ...
            else if (highestIndex >= totalDatEnteries)
            {
                highestIndex = totalDatEnteries;
                lowestIndex = totalDatEnteries - 10;
            }

            Debug.Log("Leaderboard Log // Cached cell count is" + _cachedCells.Count);
            Debug.Log("Leaderboard Log // Lowest Index is :" + lowestIndex);

            List<RectTransform> sortedCells = new List<RectTransform>();

            for (int i = 0; i < _cellPool.Count; i++)
            {
                sortedCells.Add(_cellPool[i]);
            }

            sortedCells.Sort((x, y) => x.anchoredPosition.y.CompareTo(y.anchoredPosition.y));
            sortedCells.Reverse();

            List<ICell> sortedICells = new List<ICell>();
            for (int i = 0; i < sortedCells.Count; i++)
            {
                sortedICells.Add(sortedCells[i].GetComponent<ICell>());
            }

            for (int i = 0; i < sortedICells.Count; i++)
            {
                DataSource.SetCell(sortedICells[i], lowestIndex + i);
            }
        }
        
        public LeaderboardElement GetCellByPoint(int point)
        {
            List<LeaderboardElement> elements = new List<LeaderboardElement>();
            for (int i = 0; i < _cachedCells.Count; i++)
            {
                elements.Add((LeaderboardElement)_cachedCells[i]);
            }
            
            for (int i = 0; i < elements.Count; i++)
            {
                if (elements[i].Points == point)
                {
                    return elements[i];
                }
            }
        ...
        public int GetCellDistance(int index)
        {
            int distance = index - bottomMostCellIndex;
            return distance;
        }
    }
}